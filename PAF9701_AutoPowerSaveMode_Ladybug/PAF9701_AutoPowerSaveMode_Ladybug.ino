/* 
   ...
   The sketch demonstrates how to initialize the PAF9701 in auto power save mode, configure the
   temperature limit thresholds and hysteresis, configure and report the alert flags, read the data and plot 
   the properly scaled data on the serial monitor and on a 160 x 128 pixel Adafruit TFT color display. The sensor
   suspends operation in normal mode after a user-programmable time and drops successively into higher latency
   (detectMode 1 and detectMode 2) modes until an alert threshold is crossed, then returns to the normal mode. This works well
   and is an excellent way to save battery power while waiting for activity. Kudos to PixArt Imaging, it works as described.
...
int16_t TaLow = 30, TaHigh = 60, ToLow = 40, ToHigh = 60, TaHyst = 6, ToHyst = 6, pixels = 8; // set temperature thresholds (x 2 since 0.5 C/lsb) for alerts
